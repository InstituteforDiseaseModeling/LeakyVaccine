...
make.hvtn705.optim.fn <- function ( .f.hvtn705.abc, .target.stats, .target.stat.stdevs = rep( 1, length( .target.stats ) ) ) {
    function( x ) {
        ...
        .stats.matrix <- matrix( unlist( .stats ), nrow = 1 );
        .dist  <- calculate.abc.dist( .stats.matrix, .target.stats, ifelse( is.na( .target.stat.stdevs ), 1, .target.stat.stdevs ) );
        if( is.na( .dist ) || is.null( .dist ) || !is.finite( .dist ) ) {
            ## TODO: REMOVE. DEBUGGING.
            cat( paste( "WARNING: GOT distance value `", .dist, "` from calculate.abc.distance(..)", sep = "" ), fill = TRUE );
            cat( paste( "x: ", paste( x, collapse = ", " ), sep = "" ), fill = TRUE );
            cat( paste( ".f.hvtn705.abc( x ): ", paste( .stats, collapse = ", " ), sep = "" ), fill = TRUE );
        }
        return( c( dist = unname( .dist ) ) );
    }
} # make.hvtn705.optim.fn (..)
 
## Adding this so we can compute the distances for the whole 5 parameters at once conveniently.
make.optim.fn  <- function ( target.stats, target.stat.stdevs ) {
    function( x ) {
        .stats <- run.and.compute.run.stats.hvtn705( epsilon = x[ 1 ], hvtn705.log10lambda = x[ 2 ], hvtn705.log10riskmultiplier = x[ 3 ], hvtn705.highRiskProportion = x[ 4 ], hvtn705.lowRiskProportion = x[ 5 ] );
        .stats.matrix <- matrix( unlist( .stats ), nrow = 1 );
...
exploreSimResults_hvtn705.create.params.list <- function ( reac ) {
    ...
    return( params.list );
} # exploreSimResults_hvtn705.create.params.list (..)
...
runSim_hvtn705 <- function( reac = c( "numExecution" = 10 ) ) { # Use numExecution >>1000 for best results.
    ...
    #load( file = "fit.rej.hvtn705.100k.threegroup.Rda" )
    
    # Note that this is all we need to retain from the original fit.rej for phase 2:
    target.stat.stdevs <-
        ifelse( is.na( fit.rej$stats_normalization ), 1, fit.rej$stats_normalization ) * target.stat.scale.units;
    # For fit.rej.hvtn705.100k.threegroup.Rda:
    # target.stat.stdevs  <- c(
    #     "hvtn705.VE.target" = 8.187875,
    #     "hvtn705.placebo.incidence.target" = 10.149745
    # );
...
modify.x  <- function ( x, dim.x1, x1.val, dim.x2, x2.val ) {
    modified.x  <- x;
    modified.x[ dim.x1 ]  <- x1.val;
    modified.x[ dim.x2 ]  <- x2.val;
    return( modified.x );
} # modify.x (..)
...
exploreSimResults_hvtn705 <- function( reac = c( "zoomFactor" = 10 ) ) {
    ...
    # Which margins to plot
    plot.margin.x <- 1;
    plot.margin.y <- 2;

    # If we want to try zooming in we can redo plot.points. We really only need the plot.margin.x and plot.margin.y columns of it.
    close.enough.dist.x <- min(diff( plot.points[ ,plot.margin.x] )) / the.zoom.factor;
    close.enough.dist.y <- min(diff( plot.points[ ,plot.margin.y] )) / the.zoom.factor;
    plot.points[ , plot.margin.x ]  <-
        seq( from = ( x[ plot.margin.x ] - close.enough.dist.x ), to = ( x[ plot.margin.x ] + close.enough.dist.x ), by = ( ( 2 * close.enough.dist.x ) / num.plot.points ) );
    plot.points[ , plot.margin.y ]  <-
        seq( from = ( x[ plot.margin.y ] - close.enough.dist.y ), to = ( x[ plot.margin.y ] + close.enough.dist.y ), by = ( ( 2 * close.enough.dist.y ) / num.plot.points ) );
    
    distances.to.plot  <- matrix( NA, nrow = num.plot.points, ncol = num.plot.points );
    distances.to.plot.df  <- data.frame( x = unlist( lapply( 1:num.plot.points, rep, num.plot.points ) ), y = rep( 1:num.plot.points, num.plot.points ), x.val = NA, y.val = NA, dist = NA );
    ## Add the mode
    distances.to.plot.df  <- rbind( distances.to.plot.df,
                                   c( x = 0, y = 0, x.val = unname( x[plot.margin.x] ), y.val = unname( x[plot.margin.y] ), dist = unname( the.distance.fn( x ) ) ) );

    for( i in 1:num.plot.points ) {
        for( j in 1:num.plot.points ) {
            dist  <- the.distance.fn( modify.x( x, plot.margin.x, plot.points[ i, plot.margin.x ], plot.margin.y, plot.points[ j, plot.margin.y ] ) );
            distances.to.plot[ i, j ] <- dist;
            stopifnot( distances.to.plot.df[ ( ( i - 1 ) * num.plot.points ) + j, "x" ] == i );
            distances.to.plot.df[ ( ( i - 1 ) * num.plot.points ) + j, "x.val" ] <- plot.points[ i, plot.margin.x ];
            stopifnot( distances.to.plot.df[ ( ( i - 1 ) * num.plot.points ) + j, "y" ] == j );
            distances.to.plot.df[ ( ( i - 1 ) * num.plot.points ) + j, "y.val" ] <- plot.points[ j, plot.margin.y ];
            distances.to.plot.df[ ( ( i - 1 ) * num.plot.points ) + j, "dist" ]  <- dist;
        }
    }

#     ## Add all those points we already had, but maybe just those close to our mode?
## TODO: This doesn't work very well because many of those points are far in the other dimensions; we have to restrict this to points that are also close in all the other dimensions, or something.
#     additional.distances.to.plot.df <- cbind( x = NA, y = NA, x.val = fit.rej$param[ , plot.margin.x ], y.val = fit.rej$param[ , plot.margin.y ], dist = calculate.abc.dist( fit.rej$stats, as.numeric( target.stats ), target.stat.stdevs ) );
#     ## Filter so we plot just those close to our mode:
#     which.to.keep <-
#         (
#             ( additional.distances.to.plot.df[ , "x.val" ] >= ( x[ plot.margin.x ] - close.enough.dist.x ) ) &
#             ( additional.distances.to.plot.df[ , "x.val" ] <= ( x[ plot.margin.x ] + close.enough.dist.x ) ) &
#             ( additional.distances.to.plot.df[ , "y.val" ] >= ( x[ plot.margin.y ] - close.enough.dist.y ) ) &
#             ( additional.distances.to.plot.df[ , "y.val" ] <= ( x[ plot.margin.y ] + close.enough.dist.y ) )
#         );
#     # sum( which.to.keep )
#     # [1] 4065
#     # > length( which.to.keep )
#     # [1] 100000
#     additional.distances.to.plot.df.filtered <- additional.distances.to.plot.df[ which.to.keep, , drop = FALSE ];
# 
#     distances.to.plot.df.augmented  <- rbind( distances.to.plot.df, additional.distances.to.plot.df.filtered );

    # So the point is that we fix x at all but two margins and explore the region near x by plotting how the distance varies in just those two dimensions.
    .plot  <- ggplot( distances.to.plot.df, aes( x = x.val, y = y.val, col = dist, size = 1/dist  ) ) + geom_point() + xlab( names( bounds )[ plot.margin.x ] ) + ylab( names( bounds )[ plot.margin.y ] ) + xlim( c( ( x[ plot.margin.x ] - close.enough.dist.x ), ( x[ plot.margin.x ] + close.enough.dist.x ) ) ) + ylim( c( ( x[ plot.margin.y ] - close.enough.dist.y ), ( x[ plot.margin.y ] + close.enough.dist.y ) ) ) + ggtitle( paste( "Zoom ", the.zoom.factor, sep = "" ) )

    ## TODO: Something with this.
    exploration.results <- list();

    detach( name = "params.list" );

    return( .plot );
    #return( exploration.results );
} # exploreSimResults_hvtn705 (..)
