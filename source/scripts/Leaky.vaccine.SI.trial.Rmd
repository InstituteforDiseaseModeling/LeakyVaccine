...
2. Assess if this effect might contribute to the differences between the RV 144 and HVTN 702 and HVTN 705 vaccine trial outcomes.  (There has been a couple of analyses of this, and we can build on this and make future analyses of other trial results more straightforward to evaluate.)
...
beta <- 0.003   #transmission rate (per contact)
c <- 90/365    #contact rate (contacts per day)
prev <- 0.10    #needs some more consideration; viremic individuals only
lambda <- beta*c*prev
epsilon <- 0.50  #per contact vaccine efficacy
risk <- 15.0   #risk multiplier
```
...
param <- param.dcm(beta = beta, c = c, prev = prev, epsilon = epsilon, risk = risk)
init <- init.dcm(Sp = 5000, Ip = 0,
                 Sv = 5000, Iv = 0,
                 Sph = 500, Iph = 0,    #placebo, high risk
                 Spm = 3500, Ipm = 0,   #placebo, medium risk
                 Spl = 1000, Ipl = 0,    #placebo, low risk
                 Svh = 500, Ivh = 0,    #vaccine
                 Svm = 3500, Ivm = 0,   #vaccine
...
mod.det <- dcm(param, init, control)
#mod.det

```


```{r EpiModel_stochastic}

param <- param.icm(beta = beta, c = c, prev = prev, epsilon = epsilon, risk = risk)
init <- init.icm(Sp = 5000, Ip = 0,
                 Sv = 5000, Iv = 0,
                 Sph = 500, Iph = 0,    #placebo, high risk
                 Spm = 3500, Ipm = 0,   #placebo, medium risk
                 Spl = 1000, Ipl = 0,    #placebo, low risk
...
control <- control.icm(nsteps = 365*3, type = "SI", new.mod = si_ode, nsims = 100)
mod.sto <- icm(param, init, control)
#mod.sto
...
```{r plots_deterministic}

mod <- mod.manipulate(mod.det)
